...
const fetchComments = async postId => fetch(`${process.env.NEXT_PUBLIC_API_URL}comments/post/${postId}`).then(res => res.json())
            ...
            <>
                {comments?.map(comment => (
                    <div key={comment.id} className={`block my-3 ${comment.parentId && 'mr-4'}`}>
                        <div className="border p-3" style={{ maxWidth: '20rem' }}>
                            <span className="h4 d-inline-block mb-3">{comment.name}</span>
                            {comment.parentId ? <strong>{` in reply to ${parentComment.name}`}</strong> : ''}
                            <p>{comment.content}</p>
                            <span className="ml-3" style={{ fontSize: 12 }}>
                                {moment(comment.createdAt).format('D MMM, YYYY [at] HH:mm')}
                            </span>
                            <a className="text-primary font-weight-600" href="#your-comment" onClick={() => parentIdHandler(comment)}>
                                Reply
                            </a>
                        </div>
                        {comment.children.length ? <RecurtionComments comments={comment.children} parentComment={comment} /> : ''}
                    </div>
                ))}
            </>
        )
    ...
    return (
        <div className="column is-9-desktop mt-4">
            ...
            <div className="mb-5 border-top mt-4 pt-5">
                ...
                {data?.length ? <RecurtionComments comments={data} /> : <span className="has-text-warning-dark">There are no comments yet.</span>}
            </div>

            <div className="pt-5" id="your-comment">
                <h3 className="mb-5">
                    Your Comment
                    {parentComment ? ` in reply to ${parentComment.name}` : ''}
                    {parentComment && (
                        <span onClick={cancelReply} className="text-primary" style={{ cursor: 'pointer' }} title="Cancel reply">
                            &nbsp; &times;
                        </span>
                    )}
                </h3>
                <form onSubmit={handleSubmit}>
                    <div className="columns is-multiline">
                        <div className="input-group py-0 column is-12">
                            <textarea value={content} onChange={handleChange} className="input" name="content" required />
                        </div>
                        <div className="input-group py-0 column is-6-desktop">
                            <Input value={name} name="name" onChange={handleChange} className="input" type="text" placeholder="Full name" required />
                        </div>
                        <div className="input-group py-0 column is-6-desktop">
