...
const fetchProducts = (page = 1) => fetch(`${process.env.NEXT_PUBLIC_API_URL}products/?page=${page}`, { cache: 'no-store' }).then(res => res.json())
            ...
            <Section sectionType="sm" id="products">
                <Container>
                    <div className="columns is-multiline is-tablet">
                        ...
                        <Pagination elemId="products" data={data.result} count={data.count} page={page} perPage={20} pageHandler={productsPageHandler} />
                    </div>
                </Container>
            </Section>
        </>
